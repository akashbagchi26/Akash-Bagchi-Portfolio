...
function SkillChip({ name, slug, emoji, theme, visible, delay }) {
  const iconUrl = slug
    ? `https://skillicons.dev/icons?i=${slug}&theme=${theme}`
    : null;
  return (
...
function CategoryBlock({ cat, theme, catVisible, index }) {
  ...
  return (
    <div style={{ ...reveal(catVisible, index * 80) }}>
      ...
      >
        <span
          className="cat-label font-outfit"
          style={{
            background: cat.bg,
      ...
      <div
        style={{ marginTop: 28, height: 1, background: "var(--card-border)" }}
      />
    </div>
  );
}
...
export default function Skills() {
  ...
  const { ref: catsRef, visibleItems: catVisible } = useScrollRevealList(
    categories.length,
    150,
  );
  ...
  return (
    <section
      id="skills"
      style={{ padding: "80px 24px", background: "var(--bg-alt)" }}
    >
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        >
          <p className="text-g" style={{ fontSize: "0.85rem" }}>
            ...
            <strong style={{ color: "#c4b5fd" }}>GitHub Copilot</strong> for
