...
export default function Navbar() {
  ...
  const scrollTo = href => { setMenuOpen(false); document.getElementById(href.slice(1))?.scrollIntoView({ behavior: 'smooth' }); };
    ...
    <header style={{
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 24px', height:68, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <a href="#" onClick={e=>{e.preventDefault();window.scrollTo({top:0,behavior:'smooth'})}} style={{display:'flex',alignItems:'center',gap:8,textDecoration:'none'}}>
          <span className="font-outfit gradient-text" style={{fontWeight:800,fontSize:'1.35rem'}}>AB</span>
          <span style={{color:'var(--card-border)',margin:'0 2px'}}>|</span>
          <span style={{color:'var(--t4)',fontSize:'0.82rem',fontWeight:500}}>Portfolio</span>
        </a>
        ...
        <nav style={{display:'flex',alignItems:'center',gap:28}} className="desktop-nav">
          ...
          <a href="#contact" className="btn-primary" style={{padding:'9px 20px',fontSize:'0.83rem'}}
        ...
        <button onClick={()=>setMenuOpen(!menuOpen)} className="mobile-menu-btn"
          ...
          {[0,1,2].map(i=>(
            <span key={i} style={{display:'block',width:22,height:2,background:'#a78bfa',borderRadius:2,transition:'all 0.3s',
      ...
      {menuOpen && (
        <div style={{background:'var(--nav-bg)',borderTop:'1px solid var(--nav-border)',padding:'20px 24px',display:'flex',flexDirection:'column',gap:20}}>
          {navItems.map(item=>(
            <a key={item.href} href={item.href} className={`nav-link${active===item.href?' active':''}`} style={{fontSize:'1rem'}}
              onClick={e=>{e.preventDefault();scrollTo(item.href);}}>
              {item.label}
            </a>
          ))}
